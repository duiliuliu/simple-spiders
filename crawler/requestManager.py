...
class RequestManager(object):
    ...
    def has_new_request(self, level=None):
        ...
        '''
        if level:
            return self.new_request_size(level) > 0
    ...
    def new_request_size(self, level=None):
        '''
        待爬取的请求集合长度
        ...
        if level:
            ...
            return reduce(lambda x, y: self.__new_request_size(x) + self.__new_request_size(y), self.new_requests, 0)
    ...
    def __add_level(self,):
        ...
        '''
        if not self.has_new_request(self.__level):
            self.__level += 1
    ...
    def __add_new_request(self, request, add_level=1):
        ...
        if not str(self.__level+add_level) in self.new_requests:
            self.__init__(self.__level+add_level)
